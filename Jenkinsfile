pipeline {
    ...
    environment {
        def image_name="radical-feb-2024"
        def docker_tag="radical-feb-2024-v1.0"
        def Jenkins_IP="35.94.203.76" // This should be your jenkins slave IP
        def DOCKER_NETWORK="uat"
        def DOCKER_SUBNET="10.10.2.0/28"
        def DOCKERHUB = credentials('DOCKERHUB_CREDS')
        DockerHub_repo = "aamirs/radical-private-repo"
        Docker_user = "radicalfeb2024"
        Docker_vol = "my_vol"
    }
            ...
            steps {
                script {
                    try {
                        ...
                        sh 'sudo docker run -itd --restart=always --network=${DOCKER_NETWORK} --name webserver300${BUILD_NUMBER} -p 300${BUILD_NUMBER}:80 -v ${Docker_vol}:/tmp ${image_name}:${docker_tag}'
